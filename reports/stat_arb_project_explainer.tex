...
\section*{A Theoretical Null Model for the Residual World}

There is a second theoretical lens that helps explain why this project is not
just a backtest of PCA versus RMT. After common market, sector, and style
structure is filtered out, the remaining residual panel is supposed to be much
closer to stock-specific movement. But then a harder question appears: even if
there were no real alpha left in that residual world, could a constrained
portfolio still find some apparently special direction purely because the
matrix is high-dimensional and noisy?

This is where the user's current theory work fits naturally. If \(R\) denotes a
return matrix and \(w\) denotes a portfolio vector, then the realized PnL path
is just the linear image
\[
w \mapsto Rw.
\]
So a return matrix is a linear operator from portfolio space into path space.
In the Banach-space formulation, the admissible geometry \(K\) can be read as
the set of allowed portfolio shapes: leverage limits, long/short balance,
sector neutrality, sparsity, or other exposure constraints. The output
geometry \(L\) can be read as the way the PnL path is evaluated: a variance-like
size, a path norm, or another risk functional.

The lower singular value
\[
s^+_{K \to L}(R)
\]
then asks, in effect, how small the output can be over all admissible
portfolios of unit \(K\)-size. When the return matrix is replaced by a pure
noise Gaussian matrix, the theorem gives the typical scale of what constrained
portfolios should be able to do under noise alone. The key point is that this
is not an alpha theorem. Its more precise role is a no-degeneracy guarantee.
Among all admissible portfolios, the residual matrix should not have a fake
near-kernel that lets one manufacture an almost-zero residual path purely from
high-dimensional geometry. In finance language, it helps rule out the classic
overfit where a portfolio looks extremely low-risk in sample only because the
covariance structure is ill-conditioned, and then its Sharpe looks artificially
large because the denominator has collapsed.

That is why this theory is a good conceptual fit for the stat-arb project. It
supports the claim that the residual world is well-conditioned under the
portfolio constraints. It makes the backtest more defensible by controlling the
risk-geometry side of the problem. But it still says nothing by itself about
expected return or predictive content, so profitability must come from a
separate signal argument rather than from the smallest singular value alone.

So the bridge between the theory and the project is not that the theorem is a
hidden alpha engine. The bridge is that RMT filtering tries to clean the
residual world, while the Banach singular value theory explains what a cleaned
residual matrix should still look like if it were only noise under realistic
...
The repository uses a structured benchmark panel rather than a
...
This design is not trying to pretend that the repo benchmark is live market data.
...
\section*{Original Run and Repo}

The original Georgia Tech project used a broader historical equity panel that is
not redistributed. The saved summary documents the following approximate
...
The saved materials also report max drawdown around \(8\%\) and benchmark
beta with \(|\beta_{\mathrm{SPY}}| < 0.05\). The repository therefore has
two roles:

\begin{itemize}
  \item it is a runnable benchmark implementation of the methodology
  \item it is a saved documentation layer for the original historical project
\end{itemize}
...
the repo is analogous to SPY beta control. Gross exposure is normalized,
...
turnover. The benchmark in this repo uses a 5 bps one-way cost assumption as its base
case.
...
The benchmark in this repo uses a simple train / validation / holdout structure:
...
Holdout & 2023-01-02 to 2025-06-30 & Final repo out-of-sample evaluation \\
\bottomrule
\end{tabular}
\end{center}

The selection in the repo benchmark is validation-only. Among the spectral sleeves, the chosen
final sleeve in the repo benchmark is \texttt{rmt\_filtered\_residual}. The repo also
...
The headline benchmark in this repo table is:
...
The direction of the result is clear. In the benchmark in this repo, the raw PCA
...
The cost sensitivity table in the repo benchmark is severe:
...
trading makes them fragile to execution costs. So the benchmark in this repo should
...
benchmark in this repo's bull/high-volatility and event-window recovery periods, while
...
The benchmark in this repo does not prove a live tradable stat-arb edge. It does not
...
performance claim. The historical claim is saved through summary files,
...
signals. The repository shows the full workflow: generate or load a
...
Sharpe under comparable constraints. Even where the benchmark in this repo remains
